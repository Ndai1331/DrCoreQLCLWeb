...
namespace CoreAdminWeb.Pages.QLCLCoSoNLTSKhongDuDieuKienATTP
{
    public partial class QLCLCoSoNLTSKhongDuDieuKienATTP(IBaseService<QLCLCoSoNLTSDuDieuKienATTPModel> MainService,
                                              ...
                                              IBaseService<QLCLSanPhamSanXuatModel> SanPhamService) : BlazorCoreBase
    {
        ...
        private List<Enums.TuanThuQuyDinh> TuanThuQuyDinhs { get; set; } = new List<Enums.TuanThuQuyDinh>() {
            Enums.TuanThuQuyDinh.KyCamKet,
            Enums.TuanThuQuyDinh.ChuaKyCamKet,
        };
        private List<Enums.KiemTraTuanThu> KiemTraTuanThus { get; set; } = new List<Enums.KiemTraTuanThu>() {
            Enums.KiemTraTuanThu.DaDuocKiemTra,
            Enums.KiemTraTuanThu.ChuaDuocKiemTra,
        };
        private List<Enums.TinhHinhViPham> TinhHinhViPhams { get; set; } = new List<Enums.TinhHinhViPham>() {
            Enums.TinhHinhViPham.Co,
            Enums.TinhHinhViPham.Khong,
        };
        private List<Enums.QuyMoEnum> QuyMos { get; set; } = new List<Enums.QuyMoEnum>() {
        ...
        private async Task LoadData()
        {
            ...
            BuilderQuery += "&filter[_and][1][loai][_eq]=2";
            if (!string.IsNullOrEmpty(_searchString))
            {
                ...
                BuilderQuery += $"&filter[_and][{index}][_or][3][so_giay_phep_hoat_dong][_contains]={_searchString}";
                BuilderQuery += $"&filter[_and][{index}][_or][4][co_quan_cap_giay_phep_hoat_dong][_contains]={_searchString}";
            }
            ...
            {
                index++;
                BuilderQuery += $"&filter[_and][{index}][ngay_cap_giay_phep_hoat_dong][_gte]={_fromDate?.ToString("yyyy-MM-dd")}";
            }

            if (_toDate != null)
            {
                index++;
                BuilderQuery += $"&filter[_and][{index}][ngay_cap_giay_phep_hoat_dong][_lte]={_toDate?.ToString("yyyy-MM-dd")}";
            }
        ...
        private async Task OnExportExcel()
        {
            ...
            ws.Cells[1, 4].Value = "Tuân thủ quy định";
            ws.Cells[1, 5].Value = "Kiểm tra việc tuân thủ";
            ws.Cells[1, 6].Value = "Quy mô";
            ws.Cells[1, 7].Value = "Địa chỉ";
            ws.Cells[1, 8].Value = "Số giấy phép hoạt động";
            ws.Cells[1, 9].Value = "Ngày cấp giấy phép hoạt động";
            ws.Cells[1, 10].Value = "Trạng thái";
            ...
            foreach (var item in data)
            {
                ...
                ws.Cells[row, 4].Value = item.tuan_thu_quy_dinh?.GetDescription();
                ws.Cells[row, 5].Value = item.kiem_tra_tuan_thu?.GetDescription();
                ws.Cells[row, 6].Value = item.quy_mo?.GetDescription();
                ws.Cells[row, 7].Value = item.dia_chi;
                ws.Cells[row, 8].Value = item.so_giay_phep_hoat_dong;
                ws.Cells[row, 9].Value = item.ngay_cap_giay_phep_hoat_dong?.ToString("dd/MM/yyyy");
                ws.Cells[row, 10].Value = item.status.GetDescription();
                row++;
                stt++;
            }
            ...
            // Export to browser
            var fileName = $"DanhSachCoSoNLTSKhongDuDieuKienATTP_{DateTime.Now:yyyyMMddHHmmss}.xlsx";
