...
namespace CoreAdminWeb.Pages.QLCLCoSoNLTSDuDieuKienATTPConHieuLuc
{
    public partial class QLCLCoSoNLTSDuDieuKienATTPConHieuLuc(IBaseService<QLCLCoSoNLTSDuDieuKienATTPModel> MainService,
                                              ...
                                              IBaseService<XaPhuongModel> XaPhuongService) : BlazorCoreBase
    {
        ...
        private async Task LoadData()
        {
            ...
            {
                index++;
                BuilderQuery += $"&filter[_and][{index}][ngay_het_hieu_luc][_gte]={_fromDate?.ToString("yyyy-MM-dd")}";
            }

            if (_toDate != null)
            {
                index++;
                BuilderQuery += $"&filter[_and][{index}][ngay_het_hieu_luc][_lte]={_toDate?.ToString("yyyy-MM-dd")}";
            }
        ...
        private async Task OnExportExcel()
        {
            ...
            ws.Cells[1, 5].Value = "Loại hình kinh doanh";
            ws.Cells[1, 6].Value = "Số GCN";
            ws.Cells[1, 7].Value = "Ngày cấp";
            ws.Cells[1, 8].Value = "Ngày hết hạn";
            ws.Cells[1, 9].Value = "Cơ quan cấp";
            ws.Cells[1, 10].Value = "Sản phẩm";
            ...
            foreach (var item in data)
            {
                ...
                ws.Cells[row, 4].Value = item.province?.name + ", " + item.ward?.name + ", " + item.dia_chi;
                ws.Cells[row, 5].Value = item.loai_hinh_kinh_doanh?.name;
                ...
                ws.Cells[row, 8].Value = string.Join(", ", item.chi_tiets?.Select(c => c.san_pham?.name ?? string.Empty) ?? new List<string>());
                row++;
                stt++;
            }
            ...
            // Export to browser
            var fileName = $"DanhSachCoSoNLTSDuDieuKienATTPConHieuLuc_{DateTime.Now:yyyyMMddHHmmss}.xlsx";
