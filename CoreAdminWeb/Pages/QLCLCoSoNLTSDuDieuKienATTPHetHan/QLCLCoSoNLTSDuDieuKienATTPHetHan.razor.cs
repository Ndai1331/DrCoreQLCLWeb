...
namespace CoreAdminWeb.Pages.QLCLCoSoNLTSDuDieuKienATTPHetHan
{
                                              ...
                                              IBaseService<QLCLSanPhamSanXuatModel> SanPhamService) : BlazorCoreBase
    {
        ...
        private async Task OnExportExcel()
        {
            ...
            {
                index++;
                query += $"&filter[_and][{index}][ngay_het_hieu_luc][_gte]={_fromDate?.ToString("yyyy-MM-dd")}";
            }

            if (_toDate != null)
            {
                index++;
                query += $"&filter[_and][{index}][ngay_het_hieu_luc][_lte]={_toDate?.ToString("yyyy-MM-dd")}";
            }
