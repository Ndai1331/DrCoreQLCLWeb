...
namespace CoreAdminWeb.Pages.QLCLGiaCaVatTuNN
{
    public partial class QLCLGiaCaVatTuNN(
        IBaseService<QLCLGiaCaVatTuNNModel> MainService,
        IBaseService<TinhModel> TinhService,
        IBaseService<XaPhuongModel> XaPhuongService,
        IBaseService<QLCLVatTuNongNghiepModel> VatTuNongNghiepService,
        IBaseService<DonViTinhModel> DonViTinhService) : BlazorCoreBase
    {
        private List<QLCLGiaCaVatTuNNModel> MainModels { get; set; } = new();

        private bool openDeleteModal = false;
        private bool openAddOrUpdateModal = false;
        private string _titleAddOrUpdate = "Thêm mới";
        private string _searchString = "";
        private QLCLVatTuNongNghiepModel? _selectedVatTuNongNghiepFilter { get; set; }
        ...
        private async Task LoadData()
        {
            try
            {
                ...
                {
                    index++;
                    BuilderQuery += $"&filter[_and][{index}][vat_tu_nong_nghiep][_eq]={_selectedVatTuNongNghiepFilter.id}";
                }
                ...
                if (result.IsSuccess)
                {
                    MainModels = result.Data ?? new List<QLCLGiaCaVatTuNNModel>();
                    if (result.Meta != null)
                    {
                ...
                {
                    MainModels = new List<QLCLGiaCaVatTuNNModel>();
        ...
        private async Task<IEnumerable<QLCLVatTuNongNghiepModel>> LoadVatTuNongNghiepData(string searchText)
        {
            return await LoadBlazorTypeaheadData(searchText, VatTuNongNghiepService);
        }
        ...
        private void OpenDeleteModal(QLCLGiaCaVatTuNNModel item)
        {
            try
            {
                SelectedItem = item;
        ...
        public async Task OnVatTuNongNghiepFilterChanged(QLCLVatTuNongNghiepModel? item)
        {
            _selectedVatTuNongNghiepFilter = item;
            await LoadData();
        }
        ...
        private async Task OnExportExcel()
        {
            ...
            foreach (var item in data)
            {
                ...
                ws.Cells[row, 3].Value = item.vat_tu_nong_nghiep?.name;
            ...
            // Export to browser
            var fileName = $"DanhSachGiaCaVatTuNongNghiep_{DateTime.Now:yyyyMMddHHmmss}.xlsx";
